...
def write_rpf(conn):
    ...
    for row in rows:
        ...
        # colors = matplotlib.cm.rainbow(np.linspace(0, 1, len(xaxis)))
        # X = [1, 2, 3, 4]
        # cs = [colors[i // len(X)] for i in range(len(xaxis) * len(X))]
        clr = 'r'
        """
...
if __name__ == '__main__':
    ...
    # legax = plt.axes([0.9, .8, 0.08, 0.04])
    # legax.plot(0, 0, label="80")
