...
def write_json(conn):
    ...
    for row in rows:
        ...
        f = open("MTR_" + data.id.__str__() + "_" + data.DeploymentDate.replace("-", "_").replace("/", "_") +".json", "w+")
...
def write_rpf(conn):
    ...
    for row in rows:
        ...
        data.DeploymentDepth = row[12]
        # Creating the file name based on the deployment ID and the deployment date.
        f = open("MTR_" + data.id.__str__() + "_" + data.DeploymentDate.replace("-", "_").replace("/", "_") +".rpf", "w+")
        ...
        for dat in datar:
            data.data.append(dat)
            f.write(data.data[data.data.__len__() - 1][4] + " " + data.data[data.data.__len__() - 1][3] + " " + data.data[data.data.__len__() - 1][2] + "\n")
...
def write_ODF(conn):
    ...
    for row in rows:
        ...
        # Creating the file name based on the deployment ID and the deployment date.
        f = open("MTR_" + data.id.__str__() + "_" + data.DeploymentDate.replace("-", "_").replace("/", "_") +".ODF", "w+")
        datafile = f.name
